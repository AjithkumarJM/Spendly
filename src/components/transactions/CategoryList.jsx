        ...
        <div className="mt-6 sm:mt-8">
            ...
            {categories.map((cat) => {
                ...
                return (
                    ...
                    >
                        <button
                            className="w-full flex justify-between items-center py-5 px-6 text-lg font-semibold rounded-2xl focus:outline-none transition-colors group hover:bg-blue-50 dark:hover:bg-blue-900"
                            onClick={() => setExpanded(isOpen ? null : cat)}
                            style={{ borderTopLeftRadius: 16, borderBottomLeftRadius: isOpen ? 0 : 16 }}
                        >
                            <span className="flex items-center gap-3">
                                <span
                                    className="inline-block px-3 py-1 rounded-md text-xs font-bold text-white shadow"
                                    style={{ background: borderColor }}
                                >
                                    {percent}%
                                </span>
                                <span className="text-xl font-bold" style={{ color: borderColor }}>{cat}</span>
                            </span>
                            <span className="flex items-center space-x-3">
                                <span className="font-bold text-blue-700 dark:text-blue-300 text-xl">{currency} {total}</span>
                                <span className={`transition-transform ${isOpen ? "rotate-180" : "rotate-0"}`}><ChevronDown size={28} /></span>
                            </span>
                        </button>
                        {isOpen && (
                            <div className="px-4 pb-3 pt-1 animate-fade-in">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <div className="mb-2">
                                            <h4 className="font-semibold text-blue-700 dark:text-blue-300 mb-1 text-md">Subcategories</h4>
                                            <div className="space-y-2">
                                                {Object.entries(subTxs).map(([sub, txList], idx) => {
                                                    ...
                                                    return (
                                                        <div key={sub} className="flex items-center justify-between bg-blue-50 dark:bg-blue-950 rounded-lg px-3 py-2 shadow-sm border border-blue-100 dark:border-blue-900">
                                                            <div className="flex flex-col min-w-0">
                                                                <span className="font-semibold text-blue-700 dark:text-blue-200 text-sm">{sub}</span>
                                                                {notes && <span className="text-xs italic text-gray-500 dark:text-gray-400 truncate max-w-[180px]">{notes}</span>}
                                                            </div>
                                                            <div className="flex items-center gap-2">
                                                                <span className="inline-block px-2 py-0.5 rounded-full text-xs font-bold text-white bg-gradient-to-r from-blue-500 to-blue-400 shadow">{percent}%</span>
                                                                <span className="font-bold text-blue-700 dark:text-blue-200 text-sm">{currency} {subTotal}</span>
                                                            </div>
                                                        </div>
                                                    );
                                                })}
                                    ...
                                    <div>
                                        <h4 className="font-semibold text-blue-700 dark:text-blue-300 mb-1 text-md">Spends</h4>
                                        <TransactionDayList
                                            currency={currency}
